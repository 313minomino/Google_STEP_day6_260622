...
## 考察
- 全然改良がされなくて悲しかった。
  - 改善の履歴　　https://docs.google.com/spreadsheets/d/1PBquRbrWsELnF1rT0sL8z_rSxa3r84MLRgkSUX4AOYE/edit?gid=0#gid=0
- 改良何されない理由として以下の観点が考えられる
  - 重複ペナルティと折り返しのペナルティ、独立ノードのボーナスが競合している
    - 最適なノードがどこを重視するかで変わってしまう。
      - →条件に応じてどのペナルティを重視するかの重みづけを行ったが、最適な重みを発見することができなかった。
  - 2-optが強すぎる
    - 最近傍法よりもいい経路を見つけられたとしても、2-optで改善される影響が大きすぎて結果に現れない。
      - →2-optを使わないで比較をした。
    - 想定通りの経路を見つけられても、2-optで改善されてしまう。
      - →最近傍法で見つけた経路に対して、2-optの代わりに自分のヒューリスティックで改善を行う方がいいかもしれないと思ったが、実装が間に合わなかった。
